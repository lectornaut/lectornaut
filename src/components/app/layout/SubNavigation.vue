  ...
  <div class="bg-background flex items-center justify-between gap-2 p-2">
    ...
    <div class="flex items-center gap-2">
      <Badge
        ...
        "
      >
        <icon-lucide-loader-circle v-if="running" class="animate-spin" />
        <span v-else class="relative flex size-4 items-center justify-center">
          <span
            class="absolute inline-flex size-2 rounded-full opacity-75"
            :class="active ? 'animate-ping bg-green-400' : 'bg-amber-400'"
          ></span>
          <span
