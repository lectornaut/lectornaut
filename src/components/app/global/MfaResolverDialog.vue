...
<template>
  <Dialog :open="open" @update:open="handleClose">
    <DialogContent>
      ...
      <!-- Step: verify code -->
      <div v-if="step === 'verify'" class="flex flex-col gap-4">
        <div
        ...
        <Field v-if="isTotp || smsSent" class="grid gap-2">
        ...
        <div v-if="isSms && smsSent" class="flex items-center gap-2">
          <Button
            variant="link"
            size="sm"
            class="h-auto p-0"
            ...
            @click="sendSmsCode"
          >
            {{ t("settings.account.mfa.resendCode") }}
          </Button>
        </div>
      </div>

      <DialogFooter v-if="step === 'verify' && (isTotp || smsSent)">
