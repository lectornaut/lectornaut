...
const unregisterSelection = item.registerAnswerSelection(
  answerId,
  initialDefaultChecked
)
...
watch(
  ...
  () => {
    ...
    inputElement.value.defaultChecked = controlled.value
      ? props.checked!
      : props.defaultChecked
...
<template>
  <label
    ...
    :class="
      cn(
        'border-input bg-input/20 hover:bg-input/40 data-checked:border-primary/40 data-checked:bg-primary/10 data-invalid:border-destructive has-[>input:focus-visible]:border-ring has-[>input:focus-visible]:ring-ring/50 group/questionnaire-choice relative flex min-h-11 cursor-pointer items-start gap-2.5 rounded-3xl border px-4 py-3 text-start text-sm transition-colors outline-none select-none has-[>input:focus-visible]:ring-3',
  ...
  >
    ...
    <span
      ...
      class="bg-input/90 group-data-checked/questionnaire-choice:bg-primary dark:group-data-checked/questionnaire-choice:bg-primary group-data-checked/questionnaire-choice:text-primary-foreground group-data-checked/questionnaire-choice:border-primary pointer-events-none relative flex size-4 shrink-0 translate-y-[--spacing(0.45)] items-center justify-center rounded-[5px] border border-transparent group-has-data-[slot=questionnaire-choice-description]/questionnaire-choice:translate-y-0.5 group-data-[type=radio]/questionnaire-choice:rounded-full"
    >
      <span
        data-slot="questionnaire-choice-indicator-dot"
        class="bg-primary-foreground hidden size-2 rounded-full group-data-checked/questionnaire-choice:block group-data-[type=checkbox]/questionnaire-choice:hidden dark:size-2.5"
      />
      <CheckIcon
    ...
    <span
      ...
      class="border-primary/10 bg-background/80 text-muted-foreground pointer-events-none ms-auto hidden size-5 shrink-0 translate-y-[--spacing(0.45)] items-center justify-center rounded-full border font-mono text-[0.625rem] leading-none font-medium group-has-data-[slot=questionnaire-choice-description]/questionnaire-choice:translate-y-0.5 group-data-[shortcut]/questionnaire-choice:inline-flex"
    >
      {{ shortcut }}
    </span>
  </label>
